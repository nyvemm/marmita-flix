      ...
      <v-tabs-items v-model="tab">
        <v-tab-item v-if="hasMovies">
          <Grid
            ...
            :max="-1"
            :url="`https://media-api-nyvemm.vercel.app/api/movies?s=${query}`"
            @noResults="hasMovies = false"
          />
        </v-tab-item>
        <v-tab-item v-if="hasAnimes">
          <Grid
            ...
            :max="-1"
            :url="`https://media-api-nyvemm.vercel.app/api/animes?s=${query}`"
            :search="true"
            @noResults="hasAnimes = false"
          />
        </v-tab-item>
        <v-tab-item v-if="hasGames">
          <Grid
            ...
            :fetch="parseGames"
            :max="-1"
            :url="`https://media-api-nyvemm.vercel.app/api/games?s=${query}`"
            :search="true"
            :directUrl="true"
            @noResults="hasGames = false"
          />
        </v-tab-item>
      </v-tabs-items>
    </div>
